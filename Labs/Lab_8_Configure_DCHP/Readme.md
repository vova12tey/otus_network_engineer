...
Шаг 1. Создание пула DHCPv6 и настройка интерфейса на R1

Шаг 2. Проверка на PC-A

Возникшая ошибка:
После команды ipconfig /renew выходила ошибка DHCP request failed, а в ipconfig /all DNS-серверы оставались ::.

Причина:
В Packet Tracer команда ipconfig /renew обновляет только IPv4. Для IPv6 требуется ipconfig /renew6. Кроме того, в некоторых версиях симулятора Stateless DHCPv6 не поддерживается автоматически.

Решение:
Выполнена команда ipconfig /release6 и ipconfig /renew6.
...
Шаг 1. Создание пула для сети PC-B
